...
from .data_service import TeacherDataService
...
class TeacherListFilteredView(APIView):
  def get(self, request):
      ...
      if name:
          teacher = teacher.filter(name__icontains=name)
          # same as below
          # teacher = Teacher.objects.filter(name=name)
      email = request.query_params.get("email")
      if email:
          teacher = teacher.filter(email=email)
          # same as below
      ...
      paginated_teachers = paginator.paginate_queryset(teacher, request)
